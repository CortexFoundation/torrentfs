					...
					if b, err := bencode.Marshal(t.Torrent.Info()); err == nil {
						...
						if tm.kvdb != nil && tm.kvdb.Get([]byte(SEED_PRE+t.InfoHash())) == nil {
							...
							go func(t *caffe.Torrent, fn func(t *caffe.Torrent) bool) {
								...
								case suc := <-ch:
									if !suc {
										log.Warn("Uncompleted jobs", "ih", t.InfoHash(), "suc", suc, "job", j.ID(), "ready", common.PrettyDuration(time.Duration(mclock.Now()-t.Birth())), "elapse", common.PrettyDuration(time.Duration(mclock.Now()-j.Birth())))
									} else {
										log.Info("Job has been completed", "ih", t.InfoHash(), "suc", suc, "job", j.ID(), "ready", common.PrettyDuration(time.Duration(mclock.Now()-t.Birth())), "elapse", common.PrettyDuration(time.Duration(mclock.Now()-j.Birth())))
									}
								//case <-ctx.Done():
								case <-tm.closeAll:
