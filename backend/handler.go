				...
				case <-t.GotInfo():
					if b, err := bencode.Marshal(t.Torrent.Info()); err == nil {
						...
						if tm.badger != nil && tm.badger.Get([]byte(SEED_PRE+t.infohash)) == nil {
							...
							log.Info("Imported new seed", "ih", t.infohash, "request", common.StorageSize(t.bytesRequested), "good", params.IsGood(t.infohash), "pending", len(tm.pendingTorrents), "elapsed", common.PrettyDuration(elapsed))
